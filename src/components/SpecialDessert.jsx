...
export default function SpecialDessert () {
    ...
    useEffect(() => {
        axios.get('https://dev-annaeugeniyevna-coffea.pantheonsite.io/wp-json/wp/v2/dessert?_embed&per_page=100&orderby=date&order=desc')
        .then(res => {
            setCoffees(res.data);
            setLoading(false);
        })
                ...
                style={{WebkitOverflowScrolling: 'touch'}}>
                    {coffees.map((coffee) => {
                        ...
                        return (
                            <div
                            key={coffee.id}
                            className="px-4 py-4 bg-off-white border-2 border-beige rounded-2xl snap-start">
                                <img
                                src={imageUrl}
                                alt={title}
                                className="max-w-[234px] mb-4"/>
                                <h3
